...
import { aiAgentsService } from '@/lib/aiAgents';
...
const Automation = () => {
  ...
  const { isOpen, context, agentType, openAssistant, closeAssistant } = useAIAssistant();
  ...
  const generateAIRecommendations = async () => {
    ...
    try {
      ...
      const response = await aiAgentsService.automateTaskRecommendations('automation_optimization', automationData);
  ...
  const openAIAssistant = (workflowId?: string) => {
    const workflow = workflowId ? automationWorkflows.find(w => w.id === workflowId) : null;
    const contextText = workflow 
      ? `سير العمل: ${workflow.name} - ${workflow.description}. الحالة: ${workflow.status}. معدل النجاح: ${workflow.successRate}%`
      : 'صفحة الأتمتة الذكية - إدارة وتحسين سير العمل التلقائي';
    
    openAssistant(contextText, 'business');
  };
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-3">
                ...
                >
                  ...
                  {isLoadingRecommendations ? 'جاري التحليل...' : 'توصيات ذكية'}
                </Button>
                <Button 
